...
在企业级应用中，联邦查询引擎必须具备严格的安全控制能力，防止“删库跑路”或资源耗尽。

#### 2.6.1 黑名单机制 (Blacklist Mechanism)

我们需要多层次的防御体系：

1.  **操作级黑名单 (Operation Blacklist)**:
    *   **拦截点**: 在逻辑计划 (`LogicalPlan`) 生成之后，执行之前。
    *   **策略**: 遍历计划树，如果发现 `DropTable`, `Truncate`, `Delete`, `Insert` 等节点，且配置为“只读模式”，则直接报错拒绝。
    *   **默认**: 联邦查询默认应为 **Read-Only**。

2.  **表级黑白名单 (Table Access Control)**:
    *   **白名单 (Whitelist)**: 仅允许查询明确列出的表（适合对外开放的场景）。
    *   **黑名单 (Blacklist)**: 明确禁止查询敏感表（如 `users_password`, `salary`, `audit_logs`）。
    *   **实现**: 在 `SchemaProvider` 加载元数据时进行过滤。如果表在黑名单中，引擎甚至不会知道它的存在（`table_exist` 返回 false）。

3.  **行/列级权限 (Row/Column Level Security)**:
    *   **列屏蔽**: 可以在元数据层将敏感列（如身份证号）标记为 Masked，查询时自动替换为 `******` 或拒绝查询。
    *   **行过滤**: 强制在所有查询的 `WHERE` 子句中追加租户 ID (`tenant_id = ?`)，防止越权访问。

#### 2.6.2 资源配额与熔断 (Resource Quotas & Circuit Breaking)

防止某个查询拖垮整个数据源：

*   **最大行数限制 (Max Rows Limit)**: 强制在生成 SQL 时追加 `LIMIT N`，防止全表扫描返回 10 亿行数据。
*   **查询超时 (Query Timeout)**: 设置 `sqlx` 或 HTTP 请求的超时时间。
*   **并发控制**: 对同一个数据源的并发连接数进行限制（Semaphore），超过限制排队或拒绝。

#### 2.6.3 基于代价的熔断机制 (Cost-Based Circuit Breaking)

除了静态的配额限制，我们还需要动态的、基于 CBO (Cost-Based Optimizer) 反馈的熔断机制，以防止“虽然行数不多，但计算极其昂贵”的查询拖垮系统。

*   **代价估算 (Cost Estimation)**:
    *   在物理计划生成后，DataFusion 会计算每个算子的预估代价（CPU 周期、内存占用、I/O 字节数）。
    *   我们可以为每个 `TableProvider` (数据源) 设定一个 `MaxCost` 阈值。

*   **熔断策略**:
    *   **Pre-Execution Check**: 在查询真正执行前，遍历物理计划树。如果发现某个针对特定数据源的 `Scan` 或 `Aggregation` 算子的预估代价超过阈值（例如：预计扫描 50GB 数据或进行 10亿次哈希计算），直接拒绝执行。
    *   **报错信息**: 返回友好的错误提示，例如："Estimated cost (IO: 50GB) exceeds the limit (10GB) for source 'mysql_prod'. Please add more filters."

*   **实现方式**:
    *   利用 DataFusion 的 `OptimizerRule` 或在 `PhysicalPlanner` 之后插入一个自定义的 `AnalyzerRule` 来进行检查。

### 2.7 性能深度优化：确保索引命中 (Deep Optimization: Ensuring Index Hits)

联邦查询性能优化的核心不仅仅是“下推”，更是要保证下推后的 SQL **能走索引**。

#### 2.7.1 SARGable 谓词检查 (Search ARGument ABLE)

数据库索引通常只能处理特定形式的谓词。引擎需要尽量生成 SARGable 的 SQL。

*   **LIKE 优化**:
    *   `name LIKE 'abc%'` -> **SARGable** (前缀匹配，可走索引)。
    *   `name LIKE '%abc'` -> **Non-SARGable** (全表扫描)。
    *   **策略**: 引擎分析 `Like` 表达式，如果是左模糊匹配，生成 Warning 或拒绝（在严格模式下）。

*   **函数包裹列 (Function Wrapping)**:
    *   `WHERE YEAR(create_time) = 2023` -> **索引失效**。
    *   **重写优化**: 引擎应尝试应用代数重写规则，将其转换为范围查询：
        `WHERE create_time >= '2023-01-01' AND create_time < '2024-01-01'`。

#### 2.7.2 类型安全与隐式转换 (Type Safety & Coercion)

类型不匹配是索引失效的常见原因。

*   **案例**: MySQL 中 `varchar_col = 123` (数字) 会导致全表扫描，因为数据库会将列转为数字进行比较。
*   **对策**: 
    *   DataFusion 强类型系统通常会避免这个问题。
    *   在生成 SQL 时，必须显式生成类型转换或正确的字面量格式。
    *   例如：Rust `Utf8("123")` -> SQL `'123'` (带引号)，而不是 `123`。

#### 2.7.3 执行计划透视 (Explain Integration)

为了让 DBA 确信查询走了索引，我们需要支持透传 `EXPLAIN`。

*   **命令**: `EXPLAIN VERBOSE SELECT ...`
*   **输出**: 不仅显示 DataFusion 的物理计划，还要显示**下推给源端的最终 SQL**。
*   **高级**: 甚至可以允许用户执行 `EXPLAIN REMOTE SELECT ...`，引擎将 `EXPLAIN` 关键字下推到源端数据库执行，并将源端的执行计划（如 MySQL 的 Key 使用情况）返回给用户。

### 2.8 执行计划与子查询深度解析 (Execution Plan & Subquery Handling)

联邦查询引擎不仅仅是 SQL 转发器，更是一个完整的数据库内核。对于复杂的子查询，DataFusion 采用了一系列复杂的重写和优化策略。

#### 2.8.1 从 SQL 到物理执行 (Lifecycle of a Query)

1.  **SQL Parser**: 解析文本，生成 AST (抽象语法树)。
2.  **Logical Planner**: 结合元数据，生成初始逻辑计划。此时子查询仍以 `Subquery` 表达式的形式存在。
3.  **Analyzer & Optimizer (核心)**:
    *   **Type Coercion**: 修复类型不匹配。
    *   **Subquery Unnesting (去嵌套)**: 将 `IN`/`EXISTS` 子查询重写为 `Join` (Semi/Anti/Inner)。这是最关键的一步。
    *   **Pushdown**: 尝试将 Filter 和 Projection 推入 TableScan。
4.  **Physical Planner**: 将逻辑算子转换为物理算子 (Async Execution Plan)。

#### 2.8.2 子查询的处理策略 (Subquery Strategy)

DataFusion (及大多数 MPP 引擎) 倾向于将子查询“拉平”为 Join，因为 Join 的优化空间更大。

**场景 A: 同源子查询 (Same-Source Subquery)**
如果主查询和子查询都来自同一个数据源（例如都是 MySQL 的同一个库）：

*   **目标**: 生成包含子查询的完整 SQL 发送给源端。
*   **难点**: DataFusion 默认会把子查询拆成 Join。
*   **解决**: 我们需要在 `TableProvider` 层面识别出“这部分 Join 树完全属于同一个源”，然后利用 `TableProvider::supports_filters_pushdown` 或更高级的 `GetTableSource` 接口，将整个子查询结构逆向重组为 SQL，或者使用 View 的形式下推。
*   **生成的 SQL**: `SELECT * FROM t1 WHERE id IN (SELECT id FROM t2)`。

**场景 B: 异源子查询 (Cross-Source Subquery)**
例如：MySQL (大表) 过滤条件依赖 Postgres (小表) 的结果。
`SELECT * FROM mysql_orders WHERE user_id IN (SELECT id FROM pg_users WHERE region = 'CN')`

1.  **默认行为 (Hash Semi Join)**:
    *   并行拉取 MySQL 全量数据 和 Postgres 过滤后的数据。
    *   在 DataFusion 内存中做 Hash Join。
    *   **缺点**: MySQL 数据量大时，网络传输爆炸。

2.  **高级优化: 动态过滤注入 (Runtime Filter / Dynamic Partition Pruning)**:
    这是联邦查询的杀手锏特性。
    *   **Step 1**: 优先执行 Postgres 查询 (`SELECT id FROM pg_users ...`)。
    *   **Step 2**: 收集结果集中的 `id` (例如 1000 个 ID)。
    *   **Step 3**: 动态重写 MySQL 的执行计划。
        *   将 `id IN (1, 2, ... 1000)` 注入到 MySQL 的 `TableScan` 中。
    *   **Step 4**: 发送给 MySQL 的 SQL 变为 `SELECT * FROM orders WHERE user_id IN (1, 2, ...)`。
    *   **收益**: 利用 MySQL 的索引过滤数据，网络传输减少 99%。

#### 2.8.3 复杂相关子查询 (Correlated Subqueries)

对于相关子查询（子查询引用了外层表的列）：
`SELECT * FROM t1 WHERE exists (SELECT 1 FROM t2 WHERE t2.id = t1.id)`

*   DataFusion 的优化器规则 `DecorrelateWhereExists` 会自动将其重写为 `Semi Join`。
*   引擎只需处理标准的 Join 逻辑，无需专门处理“相关性”。

#### 2.8.4 案例分析：SQL 与执行计划详解 (Case Study: SQL & Execution Plan)

为了更直观地理解上述机制，我们通过一个具体的联邦查询案例来展示 SQL 是如何转化为最终执行计划的。

**场景**:
*   **MySQL 源 (`mysql_prod`)**: 存储订单表 `orders` (大表)。
*   **PostgreSQL 源 (`pg_ref`)**: 存储汇率表 `exchange_rates` (小表)。
*   **目标**: 计算 2023 年所有订单转换成美元后的总金额。
...
SELECT 
    o.region,
    SUM(o.amount * r.rate) as total_usd
FROM 
    mysql_prod.orders o
JOIN 
    pg_ref.exchange_rates r ON o.currency = r.currency
WHERE 
...
**执行计划演变 (Plan Evolution)**:

1.  **初始逻辑计划 (Logical Plan)**:
    *   简单的树状结构：`Aggregate` -> `Join` -> (`Filter` -> `Scan MySQL`, `Scan PG`)。

2.  **优化后的逻辑计划 (Optimized Logical Plan)**:
    *   **Predicate Pushdown (谓词下推)**: `o.order_date >= '2023-01-01'` 被推入 MySQL Scan 节点。
    *   **Projection Pushdown (投影下推)**: MySQL 只需返回 `region`, `amount`, `currency` 三列；PG 只需返回 `currency`, `rate` 两列。

3.  **关键性能质变：运行时过滤 (Runtime Filter / Dynamic Partition Pruning)**:

    **用户痛点**: 你提到的担忧非常切中要害。如果仅仅下推 `order_date`，`orders` 表仍然可能返回数百万行数据（比如全年的所有订单），导致网络拥塞和本地 OOM。
    
    **解决方案**: 为了解决这个问题，联邦引擎引入了 **Runtime Filter** 机制，将执行流程从默认的“并行拉取”变为“级联过滤”。

    *   **Step 1 (Build Side)**: 引擎识别出 `exchange_rates` 是小表（基于统计信息），**优先执行**右子树的扫描。
        *   PG SQL: `SELECT currency, rate FROM exchange_rates`
        *   结果: 拿到有效货币代码 `['USD', 'CNY', 'EUR']` (假设只配置了这三种汇率)。
    *   **Step 2 (Inject)**: 引擎将这 3 个货币代码构建成一个 `InList` 表达式（如果值很多则构建 Bloom Filter），**动态注入**到左子树（大表）的 Scan 节点中。
    *   **Step 3 (Probe Side)**: 执行左子树扫描。此时发送给 MySQL 的 SQL 发生了**质的变化**！
        *   **原始下推**: `... WHERE order_date >= '2023-01-01'`
        *   **注入后下推**: `... WHERE order_date >= '2023-01-01' AND currency IN ('USD', 'CNY', 'EUR')`
    *   **结果**: MySQL 利用 `currency` 索引（如果有）过滤掉了绝大部分非目标货币的订单，返回数据量级大幅下降。

    **注意**：Runtime Filter 虽然强大，但会引入阻塞。为了避免你担心的“计算逻辑过多导致缓慢”，我们采取**基于代价的自适应策略 (Cost-Based Adaptive Strategy)**：
    *   如果小表预估行数超过阈值（如 10000 行），或者构建 Filter 超时，引擎会**自动放弃** Runtime Filter，回退到标准的 Hash Join，优先保证响应速度。

4.  **最终物理计划 (Physical Plan)**:

```text
GlobalLimitExec: skip=0, fetch=None
  ProjectionExec: expr=[region@0 as region, SUM(o.amount * r.rate)@1 as total_usd]
    AggregateExec: mode=Final, gby=[region@0], aggr=[SUM(o.amount * r.rate)]
      AggregateExec: mode=Partial, gby=[region@0], aggr=[SUM(o.amount * r.rate)]
        CoalesceBatchesExec: target_batch_size=8192
          HashJoinExec: mode=Partitioned, join_type=Inner, on=[(currency@2, currency@0)]
            // 左子树：MySQL (大表)
            RepartitionExec: partitioning=Hash([currency@2], 16)
              ForeignScanExec: source=mysql_prod
                --> 静态下推: order_date >= '2023-01-01'
                --> (可选) 动态下推: currency IN (...) [仅当 Filter 构建极快时启用]
            // 右子树：Postgres (小表)
            RepartitionExec: partitioning=Hash([currency@0], 16)
              ForeignScanExec: source=pg_ref
                --> 下推 SQL: SELECT currency, rate FROM exchange_rates
```

#### 2.8.5 业界主流方案对比 (Industry Solutions)

针对“大表关联小表”和“跨源性能”问题，主流引擎的解决思路如下：

*   **Trino (Presto)**:
    *   **Dynamic Filtering**: 类似于我们的 Runtime Filter，但它有严格的超时机制（默认为几十毫秒）。如果小表没在规定时间内返回 Filter，大表直接开始扫描，不再等待。
    *   **优势**: 不会因为小表卡顿而拖慢整体查询。
*   **Spark SQL**:
    *   **Dynamic Partition Pruning (DPP)**: 主要针对分区列。如果 Join Key 是分区字段，Spark 会先算小表，然后只读取大表的特定分区文件。
    *   **限制**: 必须是分区表才有效。
*   **Doris / StarRocks**:
    *   **Runtime Filter**: 支持 Min/Max, In-List, BloomFilter 三种模式。会根据网络和数据量自动选择最轻量级的一种。

#### 2.8.6 执行策略可视化与用户干预 (Visibility & Intervention)

正如你所建议的，解决“慢”的最好办法是**让用户知情并选择**。我们引入 **Dry Run (预执行演练)** 机制。

1.  **Explain Preview (执行前预览)**:
    用户在执行 SQL 前，可以勾选“预览计划”。引擎会返回一个摘要：
    > "该查询将涉及 `mysql_prod.orders` (大表) 和 `pg_ref.exchange_rates` (小表)。
    > **警告**: 谓词 `order_date` 可下推，但预计 MySQL 仍需扫描 **500万行** 数据。
    > **建议**: 增加更多过滤条件，或确认是否开启 Runtime Filter (可能增加 200ms 等待时间)。"

2.  **Hint 干预 (SQL Hints)**:
    允许高级用户通过 Hint 手动控制行为，跳过引擎的自动判断：
    ```sql
    -- 强制开启 Runtime Filter，不惜等待
    SELECT /*+ RUNTIME_FILTER(mode=IN_LIST) */ ...
    
    -- 强制关闭，直接流式 Join
    SELECT /*+ DISABLE_RUNTIME_FILTER */ ...
    ```

这种机制将控制权交还给用户，既避免了引擎“自作聪明”导致的性能回退，也保证了复杂场景下的灵活性。
