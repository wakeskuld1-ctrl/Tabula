...
pub fn run_demo() {
    ...
    println!("\n[1] 栈 vs 堆 (使用 Box 智能指针):");
    {
        ...
        let heap_val = Box::new(MyData {
            id: 1,
            content: "我是堆上的数据".to_string(),
        }); // 存在堆上
    ...
    {
        let s1 = Box::new(MyData {
            id: 2,
            content: "所有权演示".to_string(),
        });
        println!("  s1 拥有数据: {:?}", s1);
    ...
    {
        let shared_data = Rc::new(MyData {
            id: 4,
            content: "共享数据".to_string(),
        });
