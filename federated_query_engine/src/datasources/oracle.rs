...
use datafusion::arrow::array::{Float64Builder, Int64Builder, StringBuilder, RecordBatch};
...
use crate::cache_manager::{CacheManager, FlightGuard, CachePolicy};
use super::sql_dialect::{SqlDialect, OracleDialect};
...
impl OracleDataSource {
    pub fn new(name: String, user: String, pass: String, conn_str: String, table_name: String) -> Self {
        Self {
            name,
            user,
            pass,
    ...
    fn get_schema(&self) -> Result<SchemaRef> {
        ...
        let stmt = conn.statement(&sql).build()
            .map_err(|e| DataFusionError::Execution(format!("Failed to prepare statement: {}", e)))?;
...
impl DataSource for OracleDataSource {
    ...
    async fn register(&self, ctx: &SessionContext) -> Result<()> {
        ...
        let table = OracleTable::new(
            ...
            schema,
            self.batch_size,
            if self.use_legacy_pagination { OracleFetchStrategy::PaginationLegacy } else { OracleFetchStrategy::Pagination12c },
        );
...
impl TableProvider for OracleTable {
    ...
    fn supports_filters_pushdown(
        ...
        let dialect = OracleDialect::new(matches!(self.fetch_strategy, OracleFetchStrategy::PaginationLegacy));
        let support = filters.iter().map(|expr| {
            match dialect.expr_to_sql(expr) {
    ...
    async fn scan(
        ...
        let where_clause = if filters.is_empty() {
            ...
            let sql_filters: Vec<String> = filters.iter()
                .map(|expr| dialect.expr_to_sql(expr))
                .collect::<Result<Vec<_>>>()
                .map_err(|e| DataFusionError::Execution(format!("Failed to generate SQL from filters: {}", e)))?;
...
impl ExecutionPlan for OracleExec {
    ...
    fn execute(
        ...
        if volatility_policy == CachePolicy::Bypass {
             println!("[OracleExec] Volatility Bypass: Table '{}' (SCN: {}) is volatile. Skipping cache.", table_name, source_scn);
        }

        // 4. L2 Cache Check (Memory)
        if volatility_policy == CachePolicy::UseCache {
            if let Some(batches) = CacheManager::get_l2(&cache_key) {
                 println!("[OracleExec] Cache Hit (L2): Reading from Memory for key {}", cache_key);
            ...
            if let Some(cache_path) = CacheManager::get_l1_file(&cache_key) {
                 println!("[OracleExec] Cache Hit (L1): Reading from Parquet for key {}", cache_key);
                 let schema = self.schema.clone();
                                     ...
                                     while let Some(batch_result) = stream.next().await {
                                         if let Ok(batch) = batch_result {
                                             ...
                                             if let Some(buf) = &mut l2_buffer {
                                                 ...
                                                 if buf.len() > 1000 { l2_buffer = None; }
                                             }
                                     ...
                                     if let Some(key) = promote_key {
                                         if let Some(buf) = l2_buffer {
                                             if !buf.is_empty() {
                                                 ...
                                                 println!("[OracleExec] Promoting L1 -> L2 for key {}", key);
                                                 CacheManager::put_l2(key, buf, cost);
                                             }
                                         }
                                     }
                                 },
                                 Err(e) => { eprintln!("[OracleExec] Corrupt cache file: {:?}", e); }
                             }
                         },
                         Err(e) => { eprintln!("[OracleExec] Failed to open cache file: {:?}", e); }
                     }
                 });
            ...
            loop {
                 match CacheManager::join_or_start_flight(cache_key.clone()) {
                     ...
                     crate::cache_manager::FlightResult::IsFollower(rx) => {
                         println!("[OracleExec] Cache Stampede Protection: Waiting for key {}", cache_key);
                             ...
                             loop {
                                 ...
                                 if status == crate::cache_manager::FlightStatus::Completed {
                                     if let Some(batches) = CacheManager::get_l2(&key_clone) {
                                          println!("[OracleExec] Singleflight: Reading from L2 for key {}", key_clone);
                                     ...
                                     if let Some(path) = CacheManager::get_l1_file(&key_clone) {
                                         println!("[OracleExec] Singleflight: Reading from L1 for key {}", key_clone);
...
fn read_oracle_data_pagination(
    ...
    let select_part = if columns.is_empty() { "1".to_string() } else { columns.clone() };

    let mut offset = 0;
    
    loop {
        // Construct SQL using Dialect
        let dialect = OracleDialect::new(matches!(fetch_strategy, OracleFetchStrategy::PaginationLegacy));
        ...
        // Builders
        let mut int_builders: Vec<Option<Int64Builder>> = col_types.iter().map(|t| 
            if let DataType::Int64 = t { Some(Int64Builder::new()) } else { None }
        ).collect();

        let mut float_builders: Vec<Option<Float64Builder>> = col_types.iter().map(|t| 
            if let DataType::Float64 = t { Some(Float64Builder::new()) } else { None }
        ).collect();

        let mut string_builders: Vec<Option<StringBuilder>> = col_types.iter().map(|t| 
            if let DataType::Utf8 = t { Some(StringBuilder::new()) } else { None }
        ).collect();

        let mut current_batch_rows = 0;

        for row_result in rows {
            let row = row_result.map_err(|e| DataFusionError::Execution(format!("Error reading row: {}", e)))?;
        ...
        let batch = build_batch(&schema, &col_types, &mut int_builders, &mut float_builders, &mut string_builders, current_batch_rows)?;
...
fn spawn_oracle_fetch_and_sidecar(
                ...
                Ok(file) => {
                    ...
                    while let Some(batch) = cache_rx.recv().await {
                        ...
                        if use_l2 {
                            ...
                            if l2_buffer.len() > 1000 { use_l2 = false; l2_buffer.clear(); }
                        }
                    }
                    
                    // Finalize
                    if writer.close().await.is_ok() {
                        if tokio::fs::rename(&temp_path, &cache_path_clone).await.is_ok() {
                            let cost = start_time.elapsed().as_millis() as u64;
                            // Register L1
                            if let Ok(meta) = std::fs::metadata(&cache_path_clone) {
                                CacheManager::put_l1(cache_key_clone.clone(), cache_path_clone, meta.len(), cost);
                                // Register L2
    ...
    tokio::task::spawn_blocking(move || {
        ...
        if let Err(e) = read_oracle_data_pagination(
            ...
            user, pass, conn_str, table_name, schema, batch_size, fetch_strategy, where_clause
        ) {
