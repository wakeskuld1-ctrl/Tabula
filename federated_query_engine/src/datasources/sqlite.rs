    ...
    fn execute(
        ...
        if volatility_policy == CachePolicy::UseCache {
            if let Some(batches) = CacheManager::get_l2(&cache_key) {
                 println!("[SqliteExec] Cache Hit (L2): Reading from Memory for key {}", cache_key);
            ...
            if let Some(cache_path) = CacheManager::get_l1_file(&cache_key) {
                 println!("[SqliteExec] Cache Hit (L1): Reading from Parquet for key {}", cache_key);
            ...
            loop {
                 match CacheManager::join_or_start_flight(cache_key.clone()) {
                     ...
                     crate::cache_manager::FlightResult::IsFollower(rx) => {
                         ...
                         println!("[SqliteExec] Cache Stampede Protection: Waiting for key {}", cache_key);
                             ...
                             loop {
                                 ...
                                 if status == crate::cache_manager::FlightStatus::Completed {
                                     ...
                                     if let Some(batches) = CacheManager::get_l2(&key_clone) {
                                          println!("[SqliteExec] Singleflight: Reading from L2 for key {}", key_clone);
                                     ...
                                     if let Some(path) = CacheManager::get_l1_file(&key_clone) {
                                         println!("[SqliteExec] Singleflight: Reading from L1 for key {}", key_clone);
...
fn read_sqlite_data(
    ...
    while let Ok(Some(row)) = rows.next() {
        ...
        if row_count >= batch_size {
            ...
            let batch = RecordBatch::try_new(schema.clone(), arrays).map_err(|e| DataFusionError::ArrowError(e, None))?;
