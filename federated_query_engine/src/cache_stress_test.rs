...
use crate::datasources::sqlite::{SqliteExec, FetchStrategy};
...
async fn create_large_test_db(path: &str, rows: usize) {
    ...
    conn.execute("CREATE TABLE IF NOT EXISTS large_table (
    ...
    let tx = conn.unchecked_transaction().unwrap();
    {
        ...
        for i in 0..rows {
            ...
            let description = format!("Order description for item {}, providing some text payload for size.", i);
            let created_at = format!("2024-01-{:02}", (i % 30) + 1);
            
            stmt.execute((i, customer_id, amount, description, created_at)).unwrap();
        }
    }
    tx.commit().unwrap();
    ...
    println!("Created DB: {} with {} rows. Size: {:.2} MB", path, rows, metadata.len() as f64 / 1024.0 / 1024.0);
}
...
async fn test_full_capabilities_stress() {
    ...
    for i in 0..concurrency {
        ...
        handles.push(tokio::spawn(async move {
            crate::cache_manager::get_metrics_registry().query_count.fetch_add(1, Ordering::Relaxed);
            let start = std::time::Instant::now();
    ...
    println!("Executed {} concurrent canonical queries in {:.2}s", concurrency, duration.as_secs_f64());
    ...
    let logger_handle = tokio::spawn(async move {
        ...
        let mut file = std::fs::OpenOptions::new().append(true).open("app_metrics_series.csv").unwrap();
        ...
        loop {
            ...
            if let Err(e) = writeln!(file, "{},{},{},{},{},{},{},{},{},{},{},{},{},{},{}", 
                ts, m.query_count, m.total_query_latency_us, m.l2_hits, m.l2_misses, 
                m.l2_read_latency_us, m.l2_lock_wait_us, m.l2_eviction_count, 
                m.l1_hits, m.l1_misses, m.l1_io_latency_us, m.l1_eviction_count, 
                m.l0_requests, m.l0_exec_latency_us, m.memory_usage
            ) {
    ...
    println!("Flooding cache with {} unique queries (Target: ~{} rows processed)...", flood_count, flood_count * rows_per_query);
    ...
    println!("Throughput: {:.2} rows/sec", total_rows as f64 / duration.as_secs_f64());
