...
use datasources::{DataSource, csv::CsvDataSource, excel::ExcelDataSource, parquet::ParquetDataSource, sqlite::SqliteDataSource};
...
async fn list_tables(State(state): State<Arc<AppState>>) -> Json<serde_json::Value> {
    ...
    if let Ok(tables) = state.metadata_manager.list_tables() {
        let json_tables: Vec<serde_json::Value> = tables.iter().map(|t| {
             serde_json::json!({
...
async fn main() {
    ...
    let (data_path_str, public_path_str, db_path_str) = if std::path::Path::new("federated_query_engine").exists() {
        // Running from workspace root
        ("federated_query_engine/data", "federated_query_engine/public", "federated_query_engine/metadata.db")
    } else {
    ...
    let metadata_manager = MetadataManager::new(db_path_str).expect("Failed to initialize metadata manager");
    ...
    if let Ok(tables) = metadata_manager.list_tables() {
        for table in tables {
            ...
            if !std::path::Path::new(&table.file_path).exists() {
                println!("Warning: File for table '{}' not found at '{}'. Cleaning up metadata.", table.table_name, table.file_path);
                if let Err(e) = metadata_manager.unregister_table(&table.catalog_name, &table.schema_name, &table.table_name) {
                    eprintln!("Failed to unregister invalid table '{}': {}", table.table_name, e);
                }
                continue;
            }
            ...
            let source: Option<Box<dyn DataSource>> = if table.source_type == "csv" {
                match CacheManager::ensure_parquet_cache(&table.file_path, "csv", None).await {
                    Ok(p) => Some(Box::new(ParquetDataSource::new(table.table_name.clone(), p))),
                    Err(e) => {
                        eprintln!("Transcoding failed for {}, using original CSV: {}", table.table_name, e);
                        Some(Box::new(CsvDataSource::new(table.table_name.clone(), table.file_path.clone())))
                    }
                }
            } else if table.source_type == "excel" {
                // Now we should have the correct sheet name from metadata
                let sheet = table.sheet_name.clone().unwrap_or_else(|| "Sheet1".to_string());
                match CacheManager::ensure_parquet_cache(&table.file_path, "excel", Some(sheet.clone())).await {
                    Ok(p) => Some(Box::new(ParquetDataSource::new(table.table_name.clone(), p))),
                    Err(e) => {
                        eprintln!("Transcoding failed for {}, using original Excel: {}", table.table_name, e);
                        Some(Box::new(ExcelDataSource::new(table.table_name.clone(), table.file_path.clone(), sheet)))
                    }
                }
            } else if table.source_type == "parquet" {
                 Some(Box::new(ParquetDataSource::new(table.table_name.clone(), table.file_path.clone())))
            ...
            if let Some(source) = source {
                if source.register(&ctx).await.is_ok() {
                    ...
                    if let Err(e) = metadata_manager.register_table(
                        &ctx, 
                        &table.catalog_name, 
                        &table.schema_name, 
                        &table.table_name, 
    ...
    if !registered_names.contains("orders") {
        ...
        if std::path::Path::new(&orders_path).exists() {
            ...
            if source.register(&ctx).await.is_ok() {
                ...
                let _ = metadata_manager.register_table(&ctx, "datafusion", "public", "orders", &orders_path, "csv", None).await;
            }
        }
    }
    ...
    if !registered_names.contains("exchange_rates") {
        let rates_path = data_dir.join("exchange_rates.csv").to_str().unwrap().to_string();
        if std::path::Path::new(&rates_path).exists() {
            let source = CsvDataSource::new("exchange_rates".to_string(), rates_path.clone());
            if source.register(&ctx).await.is_ok() {
                println!("Registered data source: {}", source.name());
                let _ = metadata_manager.register_table(&ctx, "datafusion", "public", "exchange_rates", &rates_path, "csv", None).await;
            }
        }
    }
    ...
    if excel_path.exists() && !registered_names.contains("users") {
        ...
        let source = ExcelDataSource::new("users".to_string(), path_str.clone(), "Sheet1".to_string());
        if source.register(&ctx).await.is_ok() {
            println!("Registered data source: {}", source.name());
            let _ = metadata_manager.register_table(&ctx, "datafusion", "public", "users", &path_str, "excel", Some("Sheet1".to_string())).await;
        }
    }
        ...
        if let Ok(tables) = SqliteDataSource::list_tables(meta_db_path) {
            for table_name in tables {
                ...
                if !registered_names.contains(&register_name) {
                     ...
                     if source.register(&ctx).await.is_ok() {
                         println!("Registered SQLite source: {} -> {}", register_name, table_name);
                         let _ = metadata_manager.register_table(&ctx, "datafusion", "public", &register_name, meta_db_path, "sqlite", Some(table_name)).await;
                     }
                }
            }
        }
...
async fn connect_sqlite(
    ...
    match SqliteDataSource::list_tables(&path) {
        Ok(tables) => {
            ...
            for table_name in tables {
                // Register each table
                let source = SqliteDataSource::new(table_name.clone(), path.clone(), table_name.clone());
                if source.register(&state.ctx).await.is_ok() {
                    // Persist metadata
                    let _ = state.metadata_manager.register_table(&state.ctx, "datafusion", "public", &table_name, &path, "sqlite", None).await;
                    registered.push(table_name);
                }
            }
            
            if registered.is_empty() {
                 Json(serde_json::json!({ "status": "warning", "message": "Connected but no tables found or registered", "tables": [] }))
            } else {
        ...
        Err(e) => {
            ...
            if err_msg.contains("No field named") {
                ...
                if let Some(field) = unknown_field {
                      add_log(&state.logs, format!("Detected missing field '{}', attempting fix...", field));
                      match query_rewriter::fix_query(&state.ctx, &final_sql, &field).await {
                          Ok(fixed_sql) => {
                              add_log(&state.logs, format!("Retrying with fixed SQL: {}", fixed_sql));
        ...
        Ok(df) => {
            ...
            match df.collect().await {
                Ok(batches) => {
                    ...
                    for batch in batches {
                        ...
                        for i in 0..num_rows {
                            ...
                            for j in 0..num_cols {
                                ...
                                let val_str = arrow::util::display::array_value_to_string(col, i).unwrap_or("".to_string());
                    ...
                    add_log(&state.logs, format!("Query executed successfully. {} rows returned in {}ms", rows.len(), duration.as_millis()));
        ...
        if extension == "xlsx" {
            ...
            if let Ok(sheets) = ExcelDataSource::get_sheet_names(&path_str) {
                if sheets.len() > 1 {
                    ...
                    return Json(serde_json::json!({ "status": "error", "message": "Excel file contains no sheets" }));
                }
            }
        }

        let (source, final_path, final_source_type): (Option<Box<dyn DataSource>>, String, String) = if extension == "csv" {
            match CacheManager::ensure_parquet_cache(&path_str, "csv", None).await {
                Ok(p) => (Some(Box::new(ParquetDataSource::new(table_name.clone(), p.clone()))), p, "parquet".to_string()),
                Err(e) => {
                    eprintln!("Transcoding failed for {}, falling back to CSV: {}", file_name, e);
                    (Some(Box::new(CsvDataSource::new(table_name.clone(), path_str.clone()))), path_str.clone(), "csv".to_string())
                }
            }
        } else if extension == "xlsx" {
            let sheet = single_sheet_name.clone().unwrap_or_else(|| "Sheet1".to_string());
            match CacheManager::ensure_parquet_cache(&path_str, "excel", Some(sheet.clone())).await {
                Ok(p) => (Some(Box::new(ParquetDataSource::new(table_name.clone(), p.clone()))), p, "parquet".to_string()),
                Err(e) => {
                    eprintln!("Transcoding failed for {}, falling back to Excel: {}", file_name, e);
                    (Some(Box::new(ExcelDataSource::new(table_name.clone(), path_str.clone(), sheet))), path_str.clone(), "excel".to_string())
                }
            }
        } else {
        ...
        if let Some(source) = source {
            if source.register(&state.ctx).await.is_ok() {
                ...
                let sheet_name = if extension == "xlsx" { 
                    Some(if let Some(s) = &single_sheet_name { s.clone() } else { "Sheet1".to_string() }) 
                } else { 
                    None 
                };
                
                if let Err(e) = state.metadata_manager.register_table(&state.ctx, "datafusion", "public", &table_name, &final_path, &final_source_type, sheet_name).await {
                ...
                return Json(serde_json::json!({ 
                 ...
                 return Json(serde_json::json!({ "status": "error", "message": format!("Uploaded {} but failed to register. Ensure sheet exists.", file_name) }));
            }
        }

        return Json(serde_json::json!({ "status": "ok", "message": format!("Uploaded {} (not registered, unsupported type)", file_name) }));
    }
...
async fn register_table_endpoint(
    ...
    let source: Option<Box<dyn DataSource>> = if payload.source_type == "excel" {
        let sheet = payload.sheet_name.clone().unwrap_or_else(|| "Sheet1".to_string());
        match CacheManager::ensure_parquet_cache(&payload.file_path, "excel", Some(sheet.clone())).await {
             Ok(p) => Some(Box::new(ParquetDataSource::new(payload.table_name.clone(), p))),
             Err(e) => {
                 eprintln!("Transcoding failed for {}, using original Excel: {}", payload.table_name, e);
                 Some(Box::new(ExcelDataSource::new(payload.table_name.clone(), payload.file_path.clone(), sheet)))
             }
        }
        ...
        match CacheManager::ensure_parquet_cache(&payload.file_path, "csv", None).await {
             Ok(p) => Some(Box::new(ParquetDataSource::new(payload.table_name.clone(), p))),
             Err(e) => {
                 eprintln!("Transcoding failed for {}, using original CSV: {}", payload.table_name, e);
                 Some(Box::new(CsvDataSource::new(payload.table_name.clone(), payload.file_path.clone())))
             }
        }
    ...
    if let Some(source) = source {
                if source.register(&state.ctx).await.is_ok() {
                    add_log(&state.logs, format!("Manually registered data source: {}", source.name()));
                    
                    if let Err(e) = state.metadata_manager.register_table(
                        &state.ctx,
                        "datafusion", 
                        "public", 
                        &payload.table_name, 
                        ...
                        eprintln!("Failed to persist metadata for table {}: {}", payload.table_name, e);
                    }
                    
                    return Json(serde_json::json!({ "status": "ok", "message": "Registered successfully" }));
                }
            }
...
async fn get_plan(
    ...
    if let Err(e) = &plan_result {
        ...
        if err_msg.contains("No field named") {
            ...
            if let Some(field) = unknown_field {
                  if let Ok(fixed_sql) = query_rewriter::fix_query(&state.ctx, &final_sql, &field).await {
        ...
        Ok(logical_plan) => {
            ...
            match physical_plan_result {
                Ok(physical_plan) => {
                    ...
                    let plan_text =  datafusion::physical_plan::displayable(physical_plan.as_ref()).indent(true).to_string();
