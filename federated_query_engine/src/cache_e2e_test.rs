...
mod tests {
    ...
    async fn create_test_db(path: &str) {
        ...
        conn.execute("CREATE TABLE IF NOT EXISTS test_table (id INTEGER, val TEXT)", []).unwrap();
        // Insert enough data to be noticeable but not huge
        // 1000 rows
        for i in 0..1000 {
            conn.execute("INSERT INTO test_table (id, val) VALUES (?1, ?2)", (i, format!("val_{}", i))).unwrap();
        }
    }
    ...
    async fn get_table_mtime(path: &str) -> u64 {
        ...
        metadata.modified().unwrap().duration_since(std::time::UNIX_EPOCH).unwrap().as_millis() as u64
    }
    ...
    async fn test_e2e_cache_lifecycle() {
        ...
        assert!(status.iter().any(|s| s.contains("L2 Cache (Memory) Count: 1")), "L2 should have 1 entry");
        assert!(status.iter().any(|s| s.contains("L1 Cache (Disk) Count: 1")), "L1 should have 1 entry");
        ...
        // 1. Modify Source DB
        {
            ...
            conn.execute("INSERT INTO test_table (id, val) VALUES (9999, 'new_row')", []).unwrap();
        }
