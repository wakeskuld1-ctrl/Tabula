...
use datafusion::sql::sqlparser::ast::{Statement, Expr, Value, SetExpr, TableFactor, Ident, ObjectName};
...
pub async fn rewrite_query(ctx: &SessionContext, sql: &str) -> Result<String> {
    ...
    // 3. Reconstruct SQL
    let new_sql = statements.iter().map(|s| s.to_string()).collect::<Vec<_>>().join("; ");
    Ok(new_sql)
}
...
pub async fn fix_query(ctx: &SessionContext, sql: &str, unknown_field: &str) -> Result<String> {
    ...
    Ok(statements.iter().map(|s| s.to_string()).collect::<Vec<_>>().join("; "))
}

async fn fix_query_body(ctx: &SessionContext, body: &mut SetExpr, unknown_field: &str) -> Result<()> {
    if let SetExpr::Select(select) = body {
        ...
        let matched_column = valid_columns.iter().find(|c| c.eq_ignore_ascii_case(unknown_field)).cloned();
        ...
        if ctx.table_provider(&table_name).await.is_err() {
            ...
            if let Some(catalog) = ctx.catalog("datafusion") {
                if let Some(schema) = catalog.schema("public") {
                    ...
                    if let Some(real_name) = all_tables.iter().find(|t| t.eq_ignore_ascii_case(&table_name)) {
...
async fn collect_columns(ctx: &SessionContext, relation: &TableFactor, columns: &mut HashSet<String>) -> Result<bool> {
